...
const nextApi = "http://api.ftchinese.com/v1"
const subsApi = "http://www.ftacademy.cn/api/v1";
const subsSandbox = "http://www.ftacademy.cn/api/sandbox";

const nextApiBase = isProduction 
  ? nextApi 
  : "http://localhost:8000";
const subsApiBase = isProduction 
  ? subsApi 
  : "http://localhost:8200";
...
class OrderUrlBuilder {
  constructor() {
     ...
     * For local development, it is not used.
     */
    this.sandbox = false;
  }
  ...
  build() {
    ...
    let baseUrl = subsApiBase;
    // Only under production environment will we take sandbox into account.
    if (isProduction && this.sandbox) {
      baseUrl = subsSandbox
    }
    
    return `${baseUrl}/${this.payMethod}/${this.platform}/${this.tier}/${this.cycle}`;
  }
