...
class Account {
  ...
  /**
   * @param {string} url - API url with query paramters. 
   * @returns {Promise<IAliWebPay>}
   */
  async aliDesktopOrder(url) {
    const req = request
      .post(url)
      .query({
        "return_url": external.aliReturnUrl
      });
  ...
  async redeemGiftCard(code) {
    const req = request.put(subsApi.redeemGiftCard);
    ...
    const resp = await req.send({code});
  ...
  async unlink(anchor) {
    const resp = await request.delete(nextApi.linkWx)
      .set(KEY_UNION_ID, this.unionId)
      .set(KEY_USER_ID, this.id)
      .send({anchor});
  ...
  buildMailTo() {
    ...
    if (this.isMember()) {
      params.set("subject", `${this.member.buildMemberType()}_${this.member.expireDate}`)
    }
